// ─── Deterministic State Hashing for V2 Engine ───
//
// Canonical serialization of EngineStateV2 → sha256 hex string.
...
import type { EngineStateV2 } from '../../../src/core/v2/types_v2.js'
...
/**
 * Serialize EngineStateV2 into a deterministic Uint8Array.
 * Field order is fixed and explicit — no dependency on JS object key ordering.
 */
export function serializeStateDeterministic(state: EngineStateV2): Uint8Array {
  const parts: string[] = []
  ...
  for (let i = 0; i < state.orbs.length; i++) {
    ...
    // Trait multipliers (bigint → string, all 5 fields)
  ...
  // TetherV2 fields: anchorX, anchorY, protect, rest? (color is render-only)
 ...
 */
export function hashStateV2(state: EngineStateV2): string {
 ...
 */
export function assertInvariants(state: EngineStateV2, frameLabel?: string): void {
  const ctx = frameLabel ?? `frame ${state.frame}`
  for (let i = 0; i < state.orbs.length; i++) {
    const o = state.orbs[i]
    if (!Number.isSafeInteger(o.vx)) {
      throw new Error(`[${ctx}] orb ${i} vx=${o.vx} is NOT a safe integer`)
    }
    if (!Number.isSafeInteger(o.vy)) {
      throw new Error(`[${ctx}] orb ${i} vy=${o.vy} is NOT a safe integer`)
    }
    if ((o.x | 0) !== o.x) {
      throw new Error(`[${ctx}] orb ${i} x=${o.x} does not fit int32`)
    }
    if ((o.y | 0) !== o.y) {
      throw new Error(`[${ctx}] orb ${i} y=${o.y} does not fit int32`)
    }
